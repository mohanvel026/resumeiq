                ...
                <div className="h-full flex flex-col lg:flex-row gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 -mt-2">
                  ...
                  <div className="w-full lg:w-5/12 flex flex-col h-[calc(100vh-120px)] min-h-[600px] bg-white dark:bg-[#111318] border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden transition-colors duration-300">
                    ...
                    <div className="flex-1 overflow-y-auto p-5 custom-scrollbar space-y-8">
                      ...
                      <section>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div><label className={LabelCls}>Name</label><input className={InputCls} value={editData.personalInfo?.name||''} onChange={(e)=>updateField('personalInfo', null, 'name', e.target.value)} /></div>
                          <div><label className={LabelCls}>Email</label><input className={InputCls} value={editData.personalInfo?.email||''} onChange={(e)=>updateField('personalInfo', null, 'email', e.target.value)} /></div>
                          <div><label className={LabelCls}>Phone</label><input className={InputCls} value={editData.personalInfo?.phone||''} onChange={(e)=>updateField('personalInfo', null, 'phone', e.target.value)} /></div>
                          <div><label className={LabelCls}>Location</label><input className={InputCls} value={editData.personalInfo?.location||''} onChange={(e)=>updateField('personalInfo', null, 'location', e.target.value)} /></div>
                        </div>
                      </section>
                            ...
                            <div className="space-y-4">
                              {items.map((item, idx) => (
                                <div key={idx} className="bg-slate-50 dark:bg-[#0B0D10] border border-slate-200 dark:border-slate-800/80 rounded-xl p-4 relative group">
                                  <button onClick={() => removeItem(sectionKey, idx)} className="absolute top-3 right-3 text-slate-400 hover:text-rose-500 transition-colors"><Trash2 className="w-3.5 h-3.5"/></button>
                    ...
                    <div className="p-4 border-t border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#111318] flex items-center justify-between shrink-0">
                      <button className="text-[13px] font-semibold text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white" onClick={() => setStep(3)}>Back</button>
...
const topHalf = fileContent.substring(0, startIdx);
